...
function turmaList(){return Object.keys(window.TurmasETE?.getTurmas?.()||{}).sort((a,b)=>a.localeCompare(b,'pt-BR',{numeric:true}));}
...
function patchSaveDuplicateBlock(){
 const app=A(); if(!app || app.__v605SavePatch)return;
 app.__v605SavePatch=true;
...
function ensurePrintNotBlank(){
 ...
 if(window.Impressao && !window.Impressao.__v605){
   window.Impressao.__v605=true;
...
function patchEvolucao(){
 if(window.Evolucao && !window.Evolucao.__v605){
   window.Evolucao.__v605=true;
...
function generatePptx(){
 ...
 slide.addText('ETE Professor José Luiz de Mendonça', {x:0.6,y:1.3,w:12,h:0.4,fontSize:18,color:'333333'});
 ...
 pptx.writeFile({fileName:'diagnostico-pedagogico-ete.pptx'});
}
...
window.CorrecoesV605={renderEvolucaoAutomatica,generatePptx,patchFichaSelectors};
})();
