(function(){
 ...
 function manualPriorities(){const raw=$('#fichaManualDesc')?.value||''; return raw.split(/[,;\s]+/).map(normDesc).filter(Boolean).map(d=>({descriptor:d,errors:1}));}
 function selectQs(priorities, disc, count, studentName){
   let qs=(B()?.selectQuestions?B().selectQuestions(priorities,disc,count*3,{balance:($('#fichaDifficulty')?.value||'balanceada')==='balanceada'}):[]);
   const difficulty=$('#fichaDifficulty')?.value||'balanceada';
   ...
   if($('#fichaAvoidUsed')?.checked && studentName){
 ...
 function buildSheet(student, idx, opts={}){
   const a=assessment(); const r=results(); const disc=a.discipline||a.disciplina||A()?.state?.settings?.discipline||'Língua Portuguesa'; const count=Number($('#fichaCount')?.value)||10;
   ...
   const key=$('#fichaIncludeKey')?.checked!==false;
   const plan=$('#fichaIncludePlan')?.checked!==false;
 ...
 function buildAll(){
   const r=results(); const scope=$('#fichaScope')?.value||'individual'; const idx=Number($('#sheetStudent')?.value); const threshold=Number($('#fichaThreshold')?.value)||60;
   let students=[];
   if(scope==='individual'){
 ...
 function render(){
   try{current=buildAll(); $('#fichaPreview').classList.remove('empty'); $('#fichaPreview').innerHTML=current.html; $('#sheetOutput').value=current.text; $('#fichaStatus').textContent=`Gerado com sucesso: ${current.count} ficha(s).`;}
   catch(e){$('#fichaStatus').textContent=e.message; $('#fichaPreview').classList.add('empty'); $('#fichaPreview').textContent='Não foi possível gerar a ficha.';}
 }
 ...
 function bind(){
   $('#fichaGenerate')&&($('#fichaGenerate').onclick=render); $('#fichaPrint')&&($('#fichaPrint').onclick=print); $('#fichaDoc')&&($('#fichaDoc').onclick=word); $('#fichaTxt')&&($('#fichaTxt').onclick=()=>{if(!current.text)render(); download('fichas-recuperacao.txt',current.text);});
   $('#fichaClearUsage')&&($('#fichaClearUsage').onclick=()=>{if(confirm('Limpar histórico de questões já usadas?')){localStorage.removeItem('ete_question_usage');$('#fichaStatus').textContent='Histórico de uso limpo.';}});
 }
