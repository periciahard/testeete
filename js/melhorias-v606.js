...
function relatorioConsolidado(){
 ...
 let txt='RELATÓRIO CONSOLIDADO — DIAGNÓSTICO PEDAGÓGICO ETE\n\n';
 const byTurma={};
 ...
 download('relatorio-consolidado-ete-v606.txt', txt);
}
function backupSimples(){
 const app=A();
 if(!app)return;
 const data=JSON.stringify(app.state||{},null,2);
 download('backup-diagnostico-ete-v606.json', data, 'application/json;charset=utf-8');
}
...
window.MelhoriasV606={relatorioConsolidado,backupSimples,painelResumoInicial};
})();
