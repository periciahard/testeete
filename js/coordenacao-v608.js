...
function relatorioTexto(){
  ...
  let out='RELATÓRIO DA COORDENAÇÃO PEDAGÓGICA — ETE\n\n';
...
function baixarRelatorio(){ baixar('relatorio-coordenacao-ete-v608.txt', relatorioTexto()); }
...
async function gerarPowerPointInstitucional(){
  ...
  function baseSlide(title, subtitle=''){
    ...
    slide.addText('ETE Professor José Luiz de Mendonça',{x:0.45,y:7.05,w:7,h:0.25,fontSize:9,color:gray});
    return slide;
  }
  ...
  await pptx.writeFile({fileName:'diagnostico-pedagogico-institucional-ete-v608.pptx'});
}

function bind(){
  renderPainel();
  $('#coordV608Atualizar') && ($('#coordV608Atualizar').onclick=renderPainel);
  $('#coordV608Txt') && ($('#coordV608Txt').onclick=baixarRelatorio);
  $('#coordV608Pptx') && ($('#coordV608Pptx').onclick=gerarPowerPointInstitucional);
...
window.CoordenacaoV608={renderPainel,gerarPowerPointInstitucional,baixarRelatorio,relatorioTexto};
})();
