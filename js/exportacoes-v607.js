...
function gerarPdfFicha(){
  ...
  gerarPdfFromHtml(html,'ficha-institucional-ete.pdf');
}
function gerarPdfImpressao(){
  const html=ensurePrintGenerated();
  gerarPdfFromHtml(html,'fichas-em-lote-ete.pdf');
}

function patchPowerPoint(){
  const old=window.MelhoriasV606?.relatorioConsolidado;
  window.gerarPowerPointETE = async function(){
    if(!window.PptxGenJS){
    ...
    pptx.title='Diagnóstico Pedagógico ETE';
    ...
    function titleSlide(title,subtitle){
      ...
      slide.addText('ETE Professor José Luiz de Mendonça',{x:0.6,y:6.75,w:10,h:0.3,fontSize:11,color:'666666'});
      return slide;
    }
    ...
    await pptx.writeFile({fileName:'diagnostico-pedagogico-ete.pptx'});
  };
}
...
function bind(){
  ...
  if(pptBtn)pptBtn.onclick=()=>window.gerarPowerPointETE();
...
window.ExportacoesV607={gerarPdfFicha,gerarPdfImpressao,gerarWordInstitucional,gerarPdfFromHtml};
})();
