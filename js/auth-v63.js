...
function saveUser(user,remember=true){
 const u={login:user.login,nome:user.nome,perfil:user.perfil,role:user.role,anos:user.anos,disciplinas:user.disciplinas,all:user.all};
 if(remember)localStorage.setItem(STORE,JSON.stringify(u)); else sessionStorage.setItem(STORE,JSON.stringify(u));
}
function findUser(login,senha){return USERS.find(u=>u.login.toLowerCase()===String(login||'').toLowerCase() && u.senha===senha)}
...
function permissionsNav(){
 ...
 if(u.role==='prof'){
   $$('[data-view="alunos"],[data-view="turmasAlunos"],[data-view="config"]').forEach(e=>e.style.display='none');
...
function renderUsersAdmin(){
 const box=$('#usuariosV64Lista'); if(!box)return;
 box.innerHTML='<div class="preview-table"><div class="preview-row"><span>Usuário</span><span>Perfil</span><span>Anos</span><span>Disciplinas</span></div>'+
 USERS.map(u=>`<div class="preview-row"><span><b>${safe(u.login)}</b><br>${safe(u.nome)}</span><span>${safe(u.perfil)}</span><span>${safe((u.anos||[]).join(', ')||'Todos')}</span><span>${safe((u.disciplinas||[]).join(', ')||'Todas')}</span></div>`).join('')+'</div>';
}
...
window.AuthV64={user,USERS,assessmentOk,turmaOk,disciplineOk,allowedTurmas,filteredAssessments,applyPermissions};
})();
