...
const HEADER='assets/header-ete-modelo.png';
...
async function gerarPdfV64(){
 ...
 if(window.html2pdf){
   ...
   await html2pdf().set({margin:0,filename:'ficha-institucional-ete-v62.pdf',image:{type:'jpeg',quality:0.98},html2canvas:{scale:2,useCORS:true,backgroundColor:'#ffffff'},jsPDF:{unit:'mm',format:'a4',orientation:'portrait'},pagebreak:{mode:['css','legacy'],avoid:['.v62-question']}}).from(page).save();
 } else {
...
async function gerarWordV64(){
 ...
 const blob=await Packer.toBlob(doc); downloadBlob('ficha-institucional-ete-v62.docx',blob);
}
...
async function gerarPptV64(){
 ...
 function base(title,sub=''){
   ...
   sl.addText('ETE Professor José Luiz de Mendonça',{x:0.55,y:7.0,w:5,h:0.18,fontSize:8,color:gray,margin:0});
   return sl;
 }
