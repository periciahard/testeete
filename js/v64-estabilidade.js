...
const A=()=>window.ETE;
function user(){return window.AuthV63?.user?.()||null}
...
function allowedAssessments(){return window.AuthV63?.filteredAssessments?.() || (A()?.state?.assessments||[])}
...
function patchBankAuthor(){
 ...
 app.save=function(){const u=user();if(u&&Array.isArray(this.state.bank)){this.state.bank=this.state.bank.map(q=>q&&typeof q==='object'?{...q,autor:q.autor||q.author||u.login,perfilAutor:q.perfilAutor||u.perfil,updatedBy:u.login,updatedAt:q.updatedAt||new Date().toISOString()}:q)}return oldSave?oldSave():localStorage.setItem('ete_diagnostico_atual',JSON.stringify(this.state))}
}
...
window.V64={renderMinhasTurmas,allowedAssessments,latestByClass};
})();
