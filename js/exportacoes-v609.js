...
const LOGOS = ["modelo-logo-1.png", "modelo-logo-2.png"];
...
async function gerarPdf(){
  ...
  try{
    ...
    await html2pdf().set({
      margin: 0,
      filename: 'ficha-institucional-ete.pdf',
...
async function gerarWord(){
  ...
  if(!window.docx){
    ...
    downloadBlob('ficha-institucional-ete.doc',blob);
    return;
  }
  ...
  downloadBlob('ficha-institucional-ete.docx',blob);
}
...
async function gerarPptProfissional(){
  ...
  pptx.author='ETE Professor José Luiz de Mendonça';
  ...
  function slideBase(title,subtitle=''){
    ...
    slide.addText('ETE Professor José Luiz de Mendonça',{x:0.5,y:7.04,w:6.5,h:0.2,fontSize:8.5,color:gray});
    return slide;
  }
  ...
  slide.addText('ETE Professor José Luiz de Mendonça', {x:0.84,y:6.4,w:8,h:0.3,fontSize:14,color:'FFFFFF',margin:0});
  ...
  slide.addText(`Melhor desempenho atual: ${melhor?.a?.turma||'-'} (${melhor?.r?.summary?.avg||0}%).\nTurma de maior atenção: ${pior?.a?.turma||'-'} (${pior?.r?.summary?.avg||0}%).\nA média institucional considera todas as avaliações salvas localmente.`, {x:0.78,y:3.5,w:11.7,h:1.2,fontSize:16,color:'263238',breakLine:false});
  ...
  await pptx.writeFile({fileName:'diagnostico-pedagogico-profissional-ete-v609.pptx'});
}
...
function bind(){
  ...
  const pptButtons=['#generatePptxReport','#coordenacaoPptxBtn','#coordV608Pptx'];
...
window.ExportacoesV609={gerarPdf,gerarWord,gerarPptProfissional,fullDocHtml,docHtml};
})();
